...
```
**Answer** This file is populated with individual statistics for each player on each NBA team, listed in alphabetical order. It contains well-known statistics (pts, rebs, asts, blocks, etc.) and lesser known stats, like win share (off and def), usage rate and true shooting percentages.
...
**Answer** This shows the distribution of players in the NBA. This makes sense as there seems to be a somewhat even distribution among the five "major" positions, with the exception of hybrid role players.
...
**Answer** One complicating factor is how do we define how a player satisfies the conditions of a position? Where do we draw the line for what a combo guard is or a small forward/shooting guard hybrid? You would have to put a statistical baseline whether it's shooting percentage or based on characteristics like height. 
...
**ANSWER** To me, The most interesting position is the SG-PG combo because players in that position are playing a large amount of minutes and have the highest min minutes by far. Why might teams covet minutes from that player archetype?