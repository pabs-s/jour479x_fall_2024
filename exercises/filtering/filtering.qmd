...
The first result after filtering for at least 100 minutes played is TJ NeSmith from Radford. His trueshooting percentage is listed at 77.4%.
...
attendance |> 
  ...
  select(Institution, Conference, `2023`) |> 
  arrange(desc(`2023`)) |> 
  top_n(10)
```
...
matches_2023 |>
  filter(team == 'Maryland Terrapins, Terps', date <= '2023-08-31') |>
  group_by(team) |>
  summarise(
    total_attempts = sum(sh_att),
    total_on_goal = sum(so_g),
...
Overall, there is a marginal change in the Terps' performance through the first five games of the 2023 and 2024 seasons. Through the first five games in 2023, the Terps produced 56 total shot attempts with 30 of those on goal, which calculates to 53.57% of shot attempts on goal. Over the same period for the 2024 season, the Terps produced 65 total shot attempts with 31 of those on goal, which calculates to 47.69% of total shot attempts on goal. 

There is a minor uptick this year in shot attempts, but the total shots on goal is nearly identical to last year. This contributes to an almost 6% decrease this year in terms of total shots on goal through the first five games. It's hard to make any declarations about improved performance.
