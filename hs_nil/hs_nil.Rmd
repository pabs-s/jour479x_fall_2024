...
plot_usmap(data = nil_usmap_data, values = "employee_response", regions = "states") +
  scale_fill_manual(values = c("Y" = "blue", "N" = "red"), na.value = "lightgray") +
  labs(title = "These States Played Ball", subtitle = "Visual distribution of states that provided a policy response to our reporters.", fill = "Employee Response") +
  theme(panel.background = element_blank())
```
Notes:
...
pg_sn <- nil_usmap_data |>
  mutate(parent_notification_combo = case_when(
...
combo_colors <- c("Y_Y" = "blue", "Y_N" = "purple", "N_Y" = "yellow", "N_N" = "red")
...
plot_usmap(data = pg_sn, values = "parent_notification_combo", regions = "states") +
  ...
  labs(title = "Parent/Guardian and Notification Requirements in NIL Policies", fill = "Requirement") +
  theme(
    panel.background = element_blank(),
    legend.position = "left",          # Moves the legend to the left side of the plot
    legend.justification = "left",   # Centers the legend vertically on the left side
    plot.margin = margin(10, 10, 10, 30)  # Adds space on the left to avoid clipping
  )
```
